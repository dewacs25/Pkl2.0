...
class AdminLogin extends Controller
{
    ...
    public function login_action(Request $req)
    {
        ...
        if (Auth::guard('admin')->attempt(['email' => $req->email, 'password' => $req->password, 'role' => 'super-admin'])) {
            $req->session()->regenerate();
            return redirect()->intended('/admin');
        } elseif (Auth::guard('admin')->attempt(['email' => $req->email, 'password' => $req->password, 'role' => 'admin'])) {
            $req->session()->regenerate();
            return redirect()->intended('/admin');
        }
